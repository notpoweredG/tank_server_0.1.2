const api_port = "http://127.0.0.1:3000"

async function updateUI() {
    try {
        const response = await fetch(`${api_port}/api/state`);
        ...
        document.querySelectorAll('.button').forEach(button => {
            ...
            if(direction == "stop" && state.isMoving == false) isActive = true;
...
async function handleCommand(side, direction) {
    try {
        const response = await fetch(`${api_port}/api/control`, {
            method: 'POST',
